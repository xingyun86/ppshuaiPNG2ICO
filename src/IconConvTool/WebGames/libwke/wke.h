...
typedef void* (*PFUN_FILE_OPEN) (const char* path);
typedef void (*PFUN_FILE_CLOSE) (void* handle);
typedef size_t (*PFUN_FILE_SIZE) (void* handle);
typedef int (*PFUN_FILE_READ) (void* handle, void* buffer, size_t size);
typedef int (*PFUN_FILE_SEEK) (void* handle, int offset, int origin);

WKE_API void wkeSetFileSystem(PFUN_FILE_OPEN pfn_open, PFUN_FILE_CLOSE pfn_close, PFUN_FILE_SIZE pfn_size, PFUN_FILE_READ pfn_read, PFUN_FILE_SEEK pfn_seek);
