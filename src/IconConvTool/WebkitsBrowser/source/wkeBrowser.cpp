                     ...
                     int       nCmdShow)
{
	...
	t3.Start();
	//http://localhost:8888/ConvIconTool.html
	g_webView->loadURL((const utf8 *)UnicodeToUTF8(L"http://localhost:58888/ConvIconTool.html\x00").c_str());
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
   ...
   SetWindowText(hMainWnd, _T("ICO/PNG转换工具(支持透明效果)--(作者:xingyun86,秋秋:523381005)"));
...
LRESULT CALLBACK UrlEditProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    if (message == WM_CHAR && wParam == 13) //Enter Key
    {
        ...
        *((LPWORD)url) = MAX_URL_LENGTH;
		...
		g_webView->loadURL((const WCHAR *)url);
